...
# -------------------- Deudas --------------------
    ...
    if debts:
        with st.expander("✏️ Actualizar deudas guardadas"):
            for d in debts:
                st.markdown(f"**{d['name']} · {money(d['saldo'])}**")
                with st.form(f"edit_{d['id']}"):
                    s2=money_input("Saldo",d["saldo"],f"es_{d['id']}")
                    c2=money_input("Cuota",d["cuota"],f"ec_{d['id']}")
                    estado=st.selectbox(
                        "Estado",["Activa","En negociación","Pagada","Vencida","Suspendida"],
...
# -------------------- Plan --------------------
    ...
    plan=build_plan(get_profile(),debts)
    if plan["ingresos"]<=0:
        st.info("Primero cargá tu sueldo en Inicio.")
    elif plan["vivir"]<=0:
        st.warning("Primero completá en Inicio cuánto necesitás para vivir. No voy a recomendar adelantar deudas sin proteger ese dinero.")
    elif not debts:
        st.info("Cargá tus deudas para armar el plan.")
    else:
