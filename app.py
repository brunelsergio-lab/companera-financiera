...
# -------------------- Plan --------------------
    ...
    st.subheader("📅 Mi camino para salir de deudas")
    st.caption("No es un plan para un solo mes. Se recalcula cada vez que actualizás tus saldos, ingresos o gastos.")
    ...
    if plan["ingresos"]<=0:
        ...
        b.metric("🚀 Extra para acelerar",money(road.get("extra",0)))
        c.metric("🗓️ Horizonte",f"{road['months']} meses" if road.get("months") else "A recalcular")

        st.markdown("### 🧭 Orden sugerido")
        for i,(_,d,why) in enumerate(road["order"],1):
            st.markdown(
                f'<div class="card {"good" if i==1 else "info"}"><b>{i}. {d["name"]}</b> · {money(d["saldo"])}<br>'
                f'<span class="small">{why}</span></div>',
                unsafe_allow_html=True
            )

        if road.get("months"):
            st.markdown(
                f'<div class="card good"><b>🏁 Referencia de salida: alrededor de {road["months"]} meses</b><br>'
                '<span class="small">Es una guía de caja con los saldos y cuotas que cargaste hoy. '
                'No reproduce exactamente intereses futuros ni cambios del banco; cada actualización recalcula el camino.</span></div>',
                unsafe_allow_html=True
            )
        else:
            st.markdown(
                '<div class="card warn"><b>El horizonte todavía no es confiable</b><br>'
                '<span class="small">Con los datos actuales no puedo proyectar una fecha razonable. '
                'Revisá cuotas, saldos o buscá aumentar el margen mensual antes de acelerar pagos.</span></div>',
                unsafe_allow_html=True
            )

        if road.get("milestones"):
            st.markdown("### 🪜 Hitos estimados")
            for m in road["milestones"][:12]:
                st.markdown(
                    f'<div class="card"><b>Mes {m["month"]}: {m["name"]}</b><br>'
                    f'<span class="small">Al cerrarla liberarías aproximadamente {money(m["freed"])} por mes para la siguiente deuda.</span></div>',
                    unsafe_allow_html=True
                )

        st.markdown("### 🔁 El hábito que sostiene el plan")
        for title,desc in habit_checklist():
            st.markdown(f'<div class="card"><b>{title}</b><br><span class="small">{desc}</span></div>',unsafe_allow_html=True)

        st.markdown("### ✅ Qué hacer ahora")
        if plan["deficit"]>0:
            st.markdown(
                f'<div class="card bad"><b>Primero corregí un faltante de {money(plan["deficit"])}</b><br>'
                '<span class="small">No adelantes deuda mientras el mes no cubra lo básico y las cuotas obligatorias.</span></div>',
                unsafe_allow_html=True
            )
        else:
            if plan["aporte"]>0:
                st.markdown(
                    f'<div class="card info"><b>Protegé {money(plan["aporte"])} para tu colchón</b><br>'
                    '<span class="small">La meta es que un imprevisto no te obligue a volver a endeudarte.</span></div>',
                    unsafe_allow_html=True
                )
            if plan["target"] and plan["ataque"]>0:
                st.markdown(
                    f'<div class="card good"><b>Este ciclo: {plan["target"]["name"]}</b><br>'
                    f'<span style="font-size:1.3rem;font-weight:800">{money(plan["ataque"])}</span><br>'
                    f'<span class="small">{plan["reason"]}</span></div>',
                    unsafe_allow_html=True
                )
