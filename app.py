...
# -------------------- Deudas --------------------
    ...
    for d in open_debts(debts):
        st.markdown(f'<div class="card bad"><b>{d["name"]}</b><br><span class="small">{d.get("entity") or ""} · saldo {money(d["saldo"])} · cuota {money(d["cuota"])}</span></div>',unsafe_allow_html=True)
    with st.form("add_debt",clear_on_submit=True):
        name=st.text_input("Nombre"); entity=st.text_input("Entidad")
        saldo=money_input("Saldo ($)",key="d_sal"); cuota=money_input("Cuota mensual ($)",key="d_cuo")
        rest=st.number_input("Cuotas que faltan",min_value=0,value=0,step=1)
        cft=st.number_input("Costo financiero anual (%) — si lo conocés",min_value=0.0,value=0.0,step=1.0)
        oferta=money_input("Oferta para cancelar todo ($)",key="d_of")
        if st.form_submit_button("➕ Guardar deuda",type="primary",use_container_width=True):
            s=parse_money(saldo)
            q("debts").insert({"user_id":st.session_state.user_id,"name":name or "Deuda","entity":entity,
                               "saldo":s,"saldo_inicial":s,"cuota":parse_money(cuota),"cuotas_restantes":rest,
                               "cft":cft,"oferta":parse_money(oferta),"estado":"Activa"}).execute()
            st.rerun()
    if debts:
        ...
        for d in debts:
            with st.expander(f"{d['name']} · {money(d['saldo'])}"):
                with st.form(f"edit_{d['id']}"):
                    s=money_input("Saldo",d["saldo"],f"es_{d['id']}")
                    c=money_input("Cuota",d["cuota"],f"ec_{d['id']}")
                    estado=st.selectbox("Estado",["Activa","En negociación","Pagada","Vencida","Suspendida"],
                                        index=["Activa","En negociación","Pagada","Vencida","Suspendida"].index(d["estado"]) if d["estado"] in ["Activa","En negociación","Pagada","Vencida","Suspendida"] else 0,key=f"ee_{d['id']}")
                    if st.form_submit_button("Guardar",use_container_width=True):
                        q("debts").update({"saldo":parse_money(s),"cuota":parse_money(c),"estado":estado}).eq("id",d["id"]).execute(); st.rerun()
